...
const rules = {
  ...
  item_added: (todoistItem) => {
    return rules.itemRelatedProjectCreationRuleFor(todoistItem)
  },

  item_updated: (todoistItem) => {
    return rules.itemRelatedProjectCreationRuleFor(todoistItem)
  },

  itemRelatedProjectCreationRuleFor: (todoistItem) => {
    return rules
      .togglProjectIdFrom(todoistItem.project_id)
      .then(togglProjectId => {
        if (typeof togglProjectId === 'undefined') {
          return rules.todoist.fetchProject(todoistItem.project_id)
        }
      })
      .then(todoistProject => {
        if (typeof todoistProject !== 'undefined') {
          log.info(`Create Project "${todoistProject.name}"`)
          const togglProject = rules.togglTransformed(todoistProject)
          return rules.projectCreationRulesFor(todoistProject, togglProject)
        }
      })
  ...
  togglProjectIdFrom: (todoistProjectId) => {
    return rules
      .todoist.fetchProjectComments(todoistProjectId)
      .then(comments => {
        const timesheetLink = /\[Toggl Timesheet\]\((.+)\)/
        const comment = comments
          .find(fetchedComment => timesheetLink.test(fetchedComment.content))
        ...
        if (comment) {
          const link = comment.content.match(timesheetLink)[1]
          return rules.toggl.projectIdFrom(link)
        }
      })
  },
