...
const rules = {
  ...
  projectCreationRulesFor: (todoistProject, togglProject) => {
    log.info(`Create Todoist Task linking Toggl report`)
    return rules.toggl
  ...
  item_added: (todoistItem) => {
    return rules.createProjectOnBehalfOf(todoistItem)
  },

  createProjectOnBehalfOf: (todoistItem) => {
    return rules.toggl
      .fetchProjects()
      .then((projects) => {
        const knownProject = projects.find(fetchedProject =>
          fetchedProject.name.endsWith(` (${todoistItem.project_id})`))

        if (!knownProject) {
          const todoistProject = rules
            .todoist.getProject(todoistItem.project_id)
