...
const todoist = {
  ...
  project_unarchived: (data, consumer) => {
    log.info('Restore Toggl Project', { name: data.name })
    return consumer.onUnarchiveProject({
      'name': data.name
    })
  },

  project_deleted: (data, consumer) => {
    log.info('Delete Toggl Project', { name: data.name })
    return consumer.onDeleteProject({
      'name': data.name
    })
  },
